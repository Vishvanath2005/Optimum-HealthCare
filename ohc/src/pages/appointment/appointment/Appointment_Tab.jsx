...
import { AppointmentData } from "../../../component/Data"; // make sure data is imported
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center backdrop-blur-md bg-opacity-50 z-50">
      <div className="bg-layout-light dark:bg-layout-dark p-8 rounded-lg w-[450px] shadow-lg">
        <h2 className="text-xl font-bold mb-6 text-center dark:text-white">Edit Appointment</h2>
        
        <div className="space-y-4">
          {["tokenNo", "name", "phoneNumber", "date", "slot", "status"].map((field) => (
            <div key={field} className="flex flex-col gap-1">
              <label className="text-sm dark:text-gray-300 capitalize">{field.replace(/([A-Z])/g, ' $1')}</label>
              <input
                type="text"
                name={field}
        ...
        <table className="w-full xl:h-fit h-[703px] dark:text-white whitespace-nowrap">
          <thead>
            <tr className="font-semibold text-sm dark:bg-layout-dark bg-layout-light border-b-2 dark:border-overall_bg-dark border-overall_bg-light">
              ...
              {["Token No", "Name", "Phone Number", "Date", "Slot", "Status"].map((heading) => (
